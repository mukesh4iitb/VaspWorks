...
def attach_charges(atoms, fileobj='ACF.dat', ZVALs_dict=None, displacement=1e-4):

    if ZVALs_dict is None:
        ZVALs_dict = {}
    ...
    for line in lines:
        ...
        if i <= 1:
            ...
            atom.charge = float(words[j]) - ZVALs_dict.get(atom.symbol, 0)
...
def zval_dict(POSCAR_file, POTCAR_file):
    
    ZVALs_dict={}

    with open(POSCAR_file) as pos:
        lines = pos.readlines()
        symbols_line = lines[5]
        symbols = symbols_line.split()

    ZVALs = []
    with open(POTCAR_file) as pot:
        lines = pot.readlines()
        for line in lines:
            if "ZVAL" in line:
...
#atoms=attach_charges(atoms, fileobj='ACF.dat', ZVALs_dict=None, displacement=1e-4)
#for atom in atoms:
#    print(atom.charge)  # it will just print ACF.dat values.
#
#atoms=ase.io.read("POSCAR")
#ZVALs_dict = zval_dict("POSCAR", "POTCAR")
#atoms=attach_charges(atoms, fileobj='ACF.dat', ZVALs_dict=ZVALs_dict, displacement=1e-4)
#for atom in atoms:
#    print(atom.charge)  # it will print ACF.dat - ref_ZVAL value.

def MPS_charge_transfer(M="K"):
    print(os.getcwd())
    atoms=ase.io.read("POSCAR")
    zval_dict0=zval_dict("POSCAR", "POTCAR")
    
    attach_charges(atoms, "ACF.dat", ZVALs_dict=zval_dict0, displacement=None)
    
    polysulfide_charge=0
    for atom in atoms:
        if atom.symbol==f"{M}" or atom.symbol=="S":
